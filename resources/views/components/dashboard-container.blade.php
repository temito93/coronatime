    ...
    <main class="px-[108px]">
        <div class="mt-10">
            ...
            <div class="mt-10 border-b pb-4 border-solid border-custom-neutral-100">
                ...
                <a href="{{route('by.country')}}" class="pb-4 text-base text-custom-black {{request()->is($byCountry) || request()->is($byCountry."/*") ? 'border-b-[3px] border-custom-black font-bold' : 'border-none font-normal' }}">{{__('admin.by.country')}}</a>
            </div>
        </div>
        {{ $slot }}
    </main>
